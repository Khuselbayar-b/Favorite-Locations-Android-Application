...
class MainActivity :
    ...
    override fun onQueryTextChange(text: String): Boolean {
        val match = allPlaces.search(text)
        if (match.isEmpty()) {
            return true
        } else {
            updateShownPlaces(match)
        }
        return true
    }
    ...
    override fun longPressHelper(p: GeoPoint): Boolean {
        val longitude = p.longitude.toString()
        val latitude = p.latitude.toString()
        val startAddPlaceActivity = Intent(this, AddPlaceActivity:: class.java)
        startAddPlaceActivity.putExtra("latitude", latitude)
        startAddPlaceActivity.putExtra("longitude", longitude)
